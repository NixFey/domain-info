...
// TODO: This is way too much and super unclean because it's lifted from another codebase.
// go back and clean up later
...
func (r *Resolver) logf(format string, args ...any) {
	//fmt.Printf(strings.TrimSuffix(format, "\n")+"\n", args...)
}
...
func (r *Resolver) newState() *queryState {
	...
	rand.Shuffle(len(roots4), func(i, j int) {
		roots4[i], roots4[j] = roots4[j], roots4[i]
	})
	...
	if !r.NoIPv6 {
		...
		rand.Shuffle(len(roots6), func(i, j int) {
			roots6[i], roots6[j] = roots6[j], roots6[i]
		})
	...
	var (
		i   int
		ret = make([]netip.Addr, 0, len(roots4)+len(roots6))
	)
	for i = 0; i < len(roots4) && i < len(roots6); i++ {
		ret = append(ret, roots4[i], roots6[i])
	}
	ret = append(ret, roots4[i:]...)
	ret = append(ret, roots6[i:]...)
	rootServers = ret
...
func (r *Resolver) Resolve(ctx context.Context, name string) (addrs []netip.Addr, minTTL time.Duration, err error) {
	...
	if err4 != nil && err6 != nil {
		...
		return nil, 0, errors.Join(err4, err6)
	}
	if err4 != nil {
...
func (r *Resolver) resolveRecursiveFromRoot(
	...
	qstate *queryState,
	depth int,
	name string, // what we're querying
	qtype dns.Type,
...
func (r *Resolver) resolveRecursive(
	...
	authoritiesGlue, authoritiesNoGlue := partition(authorities, func(aa string) bool {
	...
	for _, authority := range authoritiesGlue {
		// custom: if the authority is the server we're looking for, that's good enough
		if authority == strings.TrimSuffix(name, ".")+"." {
			return glueRecords[authority], 0, nil
		}
...
func (r *Resolver) queryNameserverProto(
	...
	m.SetQuestion(strings.TrimSuffix(name, ".")+".", uint16(qtype))
